...
function init_download_links() {
    ...
    $('.download-button').on('click', 'a', function(event) {
        ...
        if (isDownloadLink) {
            ...
            if (newTab) {
                ...
                event.preventDefault();
                track_and_redirect(cmd, this.href);
            }
        }
    });
    ...
    $('#direct-download-link').on('click', function(event) {
        ...
        if (event.originalEvent === undefined) {
            ...
            event.preventDefault();
            track_and_redirect(['_trackEvent', 'Firefox Downloads', 'click',
                                channels[$(this).data('channel')]], this.href);
        }
    });
...
function track_download_link(cmd, url) {
    if (window.site.isIE) {
        track_and_popup(cmd, url);
        window.focus();
    } else {
        track_and_redirect(cmd, url);
    }
}
...
function track_and_popup(cmd, url) {
...
// HTTPS.
function track_and_redirect(cmd, url) {
